...
public class EarthquakeCityMap extends PApplet {
	...
	private void selectMarkerIfHover(List<Marker> markers) {
		...
		for (Marker marker : markers) {
			if (marker.isInside(map, mouseX, mouseY) && null == lastSelected) {
	...
	public void mouseClicked() {
		...
		if (lastClicked != null) {
			...
			if (null != lastClicked && lastClicked.getClicked()) {
				hideMarkersOutsideThreatCircle(false);
				return;
			}
			...
			if (null != lastClicked && lastClicked.getClicked()) {
				hideMarkersOutsideThreatCircle(true);
				return;
			}
		}
	...
	private void hideMarkersOutsideThreatCircle(boolean isCity) {

		// find marker within threat circle and make a set,
		// for all marker except this list hide

		if (isCity) {

			// lastClicked is city

			// hide all quakes for which selected city is
			// outside their threat circle
			...
			// hide all cities except last clicked
			for (Marker marker : cityMarkers) {
				if (!lastClicked.equals((CommonMarker) marker))
					marker.setHidden(true);
			}

		} else {

			// lastClicked is Earthquake
			// calculate threatRadii
			double threatRadii = ((EarthquakeMarker) lastClicked).threatCircle();

			// hide all cities outside threat circle
			for (Marker marker : cityMarkers) {
				if (((CityMarker) marker).getDistanceTo(lastClicked.getLocation()) > threatRadii)
					marker.setHidden(true);
			}

			// hide all earthquakes except last clicked
			for (Marker marker : quakeMarkers) {
				if (!lastClicked.equals((CommonMarker) marker))
					marker.setHidden(true);
			}
	...
	private void clickMarker(List<Marker> markers) {
		...
		for (Marker marker : markers) {
			if (marker.isInside(map, mouseX, mouseY) && null == lastClicked) {
